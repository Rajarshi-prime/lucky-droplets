...
stb = MPI_particles(comm, L, N, Nprtcl,sts, st,nu, tf,1000,M0 ,d,X,Y,Z, x,y,z)
...
def full_RHS(t,uk, n,sump, coord, stb,ku =ku,kn = kn,fc = fc,visc = 1,forc = 1):
    ...
    v[2] = irfft_mpi((uk[2] - tps*(1j*kz*pk - nu*((-lap)**lp)*uk[2]*isexplicit * visc))*dealias, v[2])
    ...
    kn[:] = irfft_mpi(-1j* (kx*vnk[0] + ky*vnk[1] + kz*vnk[2]) + fck, kn) 
        
    return ku,kn,kp,sump

    
def RK4(t,h,stb, uk,n):
    """Template on how to evolve the particle + flow system"""
    sump = 1.0*stb.coord
    sum_u = 1.0*uk
    sum_n = 1.0*n
    
    ku[:],kn[:],kp,sump = full_RHS(t,uk, n,sump,stb.coord,stb)
    sump += h/6.0*kp
    sum_u += h/6.0*ku
    sum_n += h/6.0*kn
    

    ku[:],kn[:],kp,sump = full_RHS(t + h/2,uk + ku*h/2, n + kn*h/2,sump, stb.coord + h/2 *kp, stb)
    sump += h/3.0*kp
    sum_u += h/3.0*ku
    sum_n += h/3.0*kn
    ...
    ku[:],kn[:],kp,sump = full_RHS(t + h,uk + ku*h, n + kn*h,sump, stb.coord + h *kp, stb)
    sump += h/6.0*kp
    sum_u += h/6.0*ku
    sum_n += h/6.0*kn
    
    return sump*1.0, sum_u*1.0,sum_n*1.0
...
def load_npz(paths,uk,n,tps = tps, tf = tf,loadn=  True): #! Rewrite
    ...
    for lidx,j in enumerate(rank_data):
        ...
        if slab_old != slab:  
            ...
            if loadn== True:nField = np.load(paths/f"st_{tps/tf:.3f}/n_{slab}.npz")
        slab_old = slab
...
def save(i,uk,n,stb,tf = tf, tps = tps): 
    ...
    # ----------- ----------------------------
    if Nprtcl > 0: new_dir = savePath/f"time_{t[i]:.1f}/sts_{tps/tf:.3f}_stb_{stb0:.3f}"
    else: new_dir = savePath/f"last/sts_{tps/tf:.3f}"
    ...
    if Nprtcl > 0: np.savez_compressed(f"{new_dir}/sts_{tps/tf:.3f}_stb_{stb0:.3f}/n_{rank}",n = n)
    else: np.savez_compressed(f"{new_dir}/sts_{tps/tf:.3f}/n_{rank}",n = n)
    
    if Nprtcl> 0:
        new_dir = savePath/f"time_{t[i]:.1f}/stb_{stb0:.3f}_sts_{tps/tf:.3f}"
        try: new_dir.mkdir(parents=True,  exist_ok=True)
        except FileExistsError: pass
        comm.Barrier()
...
if rank ==0 : print(f"Initial Physical space energy: {e0},{einit}")
#-------------------------------------------------------
